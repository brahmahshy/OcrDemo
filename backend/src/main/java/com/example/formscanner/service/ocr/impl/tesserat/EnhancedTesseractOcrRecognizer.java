...
public class EnhancedTesseractOcrRecognizer  extends AbstractTesseractOcrRecognizer {
    ...
    protected void setTessVariable(ITesseract tesseract) {
        ...
        tesseract.setVariable("language_model_penalty_non_dict_word", "0.2"); // 降低非词典词的惩罚
        tesseract.setVariable("language_model_penalty_non_freq_dict_word", "0.2"); // 降低非常用词的惩罚
        tesseract.setVariable("textord_min_linesize", "2.5"); // 最小行高
        tesseract.setVariable("edges_max_children_per_outline", "40"); // 每个轮廓最大子节点数
        tesseract.setVariable("edges_children_count_limit", "5"); // 子节点计数限制
        tesseract.setVariable("edges_children_fix", "true"); // 修复子节点
